...
```bash
pnpm install -g @azure-tools/typespec-client-generator-cli
```
...
{
  "scripts": {
    "generate:client": "tsp-client update -d && pnpm run format && dev-tool run customization apply-v2 --skip index.ts",
