...
flow](https://learn.microsoft.com/azure/active-directory/develop/v2-oauth2-auth-code-flow)
with Proof Key for Code Exchange (PKCE) on Node.js, and the authorization code
flow in the browser. It is primarily useful for applications that need to
authenticate interactively on behalf of a user.

To use this credential successfully, your app registration will need to have
a **Redirect URI** configured. You must also make sure to specify the same
URI in the `redirectUri` field of the `InteractiveBrowserCredentialOptions` when
creating an `InteractiveBrowserCredential`.

> NOTE: In the browser, you will need to configure the redirect URI in the
> **Authentication** page of your app registration under **Redirect URIs**.
