...
6.  Copy the `tsconfig.json`, `sample.env`(and `.env`) files that are present at the `sdk/<service>/<service-sdk>` to `sdk/<service>/<service-sdk>-perf-tests`.
    ...
    - Modify the "extends" string in the copied tsconfig by adding ".." if needed (the perf tests project is located at the same level as the actual SDK, under the service directory).
...
3. Add a `package.json` such as the track-2 package.json at `sdk/<service>/<service-sdk>-track-1-perf-tests` folder.
   ...
   Make sure to add the "setup" step in package.json that installs the track-1 dependencies.
