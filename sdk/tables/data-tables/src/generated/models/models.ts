...
export function _tableQueryResponseDeserializer(item: any): _TableQueryResponse {
  if (!item) return {} as _TableQueryResponse;
...
export function tablePropertiesDeserializer(item: any): TableProperties {
  if (!item) return {} as TableProperties;
...
export function tableResponseDeserializer(item: any): TableResponse {
  if (!item) return {} as TableResponse;
...
export function tableEntityQueryResponseDeserializer(item: any): TableEntityQueryResponse {
  if (!item) return {} as TableEntityQueryResponse;

  return {
    odataMetadata: item["odata.metadata"],
    value: !item["value"] ? item["value"] : entityValueTypeRecordArrayDeserializer(item["value"]),
  };
}

export function entityValueTypeRecordArraySerializer(
  result: Array<Record<string, EntityValueType>>,
): any[] {
  return result.map((item) => {
    return entityValueTypeRecordSerializer(item);
  });
}

export function entityValueTypeRecordArrayDeserializer(
  result: Array<Record<string, EntityValueType>>,
): any[] {
  if (!result) return [] as any[];
  return result.map((item) => {
    return entityValueTypeRecordDeserializer(item);
  });
}

export function entityValueTypeRecordSerializer(
  item: Record<string, EntityValueType>,
): Record<string, any> {
  const result: Record<string, any> = {};
  Object.keys(item).map((key) => {
    result[key] = !item[key] ? item[key] : entityValueTypeSerializer(item[key]);
  });
  return result;
}

export function entityValueTypeRecordDeserializer(
  item: Record<string, any>,
): Record<string, EntityValueType> {
  if (!item) return {} as Record<string, EntityValueType>;
  const result: Record<string, any> = {};
  Object.keys(item).map((key) => {
    result[key] = !item[key] ? item[key] : entityValueTypeDeserializer(item[key]);
  });
  return result;
}

/** The type for values of entity properties */
export type EntityValueType = string | number | number | boolean;

export function entityValueTypeSerializer(item: EntityValueType): any {
  return item;
}

export function entityValueTypeDeserializer(item: any): EntityValueType {
  return item;
}
...
export function signedIdentifiersDeserializer(item: any): SignedIdentifiers {
  if (!item) return {} as SignedIdentifiers;
...
export function signedIdentifiersXmlObjectSerializer(item: SignedIdentifiers): XmlSerializedObject {
  return {
    SignedIdentifier: item["identifiers"]?.map((i: any) => signedIdentifierXmlObjectSerializer(i)),
  };
}
...
export function signedIdentifierDeserializer(item: any): SignedIdentifier {
  if (!item) return {} as SignedIdentifier;
...
export function accessPolicyDeserializer(item: any): AccessPolicy {
  if (!item) return {} as AccessPolicy;
...
export function accessPolicyXmlSerializer(item: AccessPolicy): string {
  const properties: XmlPropertyMetadata[] = [
    { propertyName: "start", xmlOptions: { name: "Start" }, type: "date", dateEncoding: "rfc7231" },
    {
      propertyName: "expiry",
...
export function accessPolicyXmlObjectSerializer(item: AccessPolicy): XmlSerializedObject {
  return {
    Start: item["start"] !== undefined ? item["start"].toUTCString() : undefined,
    Expiry: item["expiry"] !== undefined ? item["expiry"].toUTCString() : undefined,
    Permission: item["permission"],
  };
}
...
export function tableServiceErrorDeserializer(item: any): TableServiceError {
  if (!item) return {} as TableServiceError;
...
export function tableServiceErrorXmlDeserializer(xmlString: string): TableServiceError {
  ...
  return deserializeFromXml<TableServiceError>(xmlString, properties, "TablesServiceError");
}

export function tableServiceErrorXmlObjectDeserializer(
  xmlObject: Record<string, unknown>,
): TableServiceError {
  const properties: XmlPropertyDeserializeMetadata[] = [
    {
  ...
  return deserializeXmlObject<TableServiceError>(xmlObject, properties);
}
...
export function tableServicePropertiesDeserializer(item: any): TableServiceProperties {
  if (!item) return {} as TableServiceProperties;
...
export function tableServicePropertiesXmlObjectSerializer(
  item: TableServiceProperties,
): XmlSerializedObject {
  return {
    Logging:
      item["logging"] !== undefined ? loggingXmlObjectSerializer(item["logging"]) : undefined,
    HourMetrics:
...
export function tableServicePropertiesXmlObjectDeserializer(
  xmlObject: Record<string, unknown>,
  ...
  return deserializeXmlObject<TableServiceProperties>(xmlObject, properties);
}
...
export function retentionPolicyDeserializer(item: any): RetentionPolicy {
  if (!item) return {} as RetentionPolicy;
...
export function tableServiceStatsDeserializer(item: any): TableServiceStats {
  if (!item) return {} as TableServiceStats;
...
export function tableServiceStatsXmlObjectDeserializer(
  xmlObject: Record<string, unknown>,
): TableServiceStats {
  const properties: XmlPropertyDeserializeMetadata[] = [
    {
  ...
  return deserializeXmlObject<TableServiceStats>(xmlObject, properties);
}
...
export function geoReplicationDeserializer(item: any): GeoReplication {
  if (!item) return {} as GeoReplication;
